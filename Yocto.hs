...
votingPassValidator treasury govClass nft idMaker propclass voteclass datum r ctx =
  ...
  in
      noIdsLeaveScript txOuts continuingOutputs &&
      ...
      if not (null idMakers) then
          ...
          (proposalPresent && -- This just makes sure that the proposal is present.
              correctScriptOutputs treasuryAddr txInfo txIns txOuts continuingOutputs propTxos && -- This needs to check spending from the treasury -- Also checks any minted gov tokens -> Goes to treasury. -- Prop to burn?
          ...
          (oldEnoughInput txInfo voteTxos propTxos &&
              correctPropOutputs votes txInfo txOuts)))
    where
        ...
        noIdsLeaveScript :: [TxOut] -> [TxOut] -> Bool
        noIdsLeaveScript txOuts continuingOutputs = [tx | tx <- txOuts, ((assetClassValueOf (txOutValue tx) propclass > 0) || (assetClassValueOf (txOutValue tx) voteclass > 0))] == [tx | tx <- continuingOutputs, ((assetClassValueOf (txOutValue tx) propclass > 0) || (assetClassValueOf (txOutValue tx) voteclass > 0))]
        ...
        correctPropOutputs :: Integer -> TxInfo -> [TxOut] -> Bool
        correctPropOutputs votes txInfo propTxos =
          let
              propIn = propTxos !! 0
        ...
        checkScriptTreasuryUse :: Address -> TxInfo -> [TxInInfo] -> [TxOut] -> [TxOut] -> Proposal -> Bool
        checkScriptTreasuryUse treasury txInfo txIns txOuts continuingOutputs p =
          let
              spent = spendValue p
        ...
        checkScriptMintingUse :: Address -> TxInfo -> [TxInInfo] -> [TxOut] -> [TxOut] -> Proposal -> Bool
        checkScriptMintingUse treasury txInfo txIns txOuts continuingOutputs p =
          let
              treasuryIns = [tx | tx <- txIns, ((txOutAddress $ txInInfoResolved tx) == treasury)]
              nftOuts = [tx | tx <- txOuts, (assetClassValueOf (txOutValue tx) nft > 0)]
              mintVal = txInfoMint txInfo
        ...
        checkScriptUpdate :: Address -> TxInfo -> [TxInInfo] -> [TxOut] -> Proposal -> Bool
        checkScriptUpdate treasury txInfo txIns txOuts p =
          let
              treasuryIns = [tx | tx <- txIns, ((txOutAddress $ txInInfoResolved tx) == treasury)]
              nftOuts = [tx | tx <- txOuts, (assetClassValueOf (txOutValue tx) nft > 0)]
              nftOut = nftOuts !! 0
        ...
        checkScriptOutputs' :: Address -> TxInfo -> [TxInInfo] -> [TxOut] -> [TxOut] -> Proposal -> Bool
        checkScriptOutputs' treasury txInfo txIns txOuts continuingOutputs p =
            if spend p then
                checkScriptTreasuryUse treasury txInfo txIns txOuts continuingOutputs p
        ...
        correctScriptOutputs :: Address -> TxInfo -> [TxInInfo] -> [TxOut] -> [TxOut] -> [TxInInfo] -> Bool
        correctScriptOutputs treasury txInfo txIns txOuts continuingOutputs propTxos =
          let
